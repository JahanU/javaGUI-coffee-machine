...
public class CoinValidation extends MachineController {
	...
	private static int coinChange[] = new int[6]; // Stores value of change
	private static int amount[] = new int[6]; // Stores the amount of change 
	private static int coins[] = {100, 50, 20, 10, 5, 1}; // Used to loop through coins (Stored in reverse order)
	private static String coinCodes[] = {"ef", "bd", "bc", "ba", "ac", "ab"}; // Stores order code (in reverse order)
	private static String oldBalance = ""; // Used to store the current balance before resetting the current balance
	...
	private static void calcChange() { // returns highest amount of coins that is available within the machine
		...
		oldBalance = CoinValidation.convertToMoneyDisplay(); // Stores balance before resetting the balance / used to display to the user
		...
		for (int i = 0; i < coins.length; i++) {
			...
			machine.getDisplay().setTextString("Balance: " + CoinValidation.convertToMoneyDisplay()); // Shows updated balance
		}
	}
		...
		if (machine.getBalance() == 0) { 
			...
			for (int i = 0; i < coinChange.length; i++) {
				if (amount[i] > 0) { // 
					...
					machine.getDisplay().setTextString("Balance: " + CoinValidation.convertToMoneyDisplay() + " returned: " + oldBalance);
				}
			}
			for (int i = 0; i < coinChange.length; i++) {
